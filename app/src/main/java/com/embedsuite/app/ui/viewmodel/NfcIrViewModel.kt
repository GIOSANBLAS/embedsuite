...
class NfcIrViewModel(
    ...
    val nfcDeviceEnabled: StateFlow<Boolean> = combine(
        ...
        conn is ConnectionState.Connected &&
            (profile != FirmwareProfile.BRUCE || connectionManager.hasBruceCapability("nfc"))
    ...
    val irDeviceEnabled: StateFlow<Boolean> = combine(
        ...
        conn is ConnectionState.Connected &&
            (profile != FirmwareProfile.BRUCE || connectionManager.hasBruceCapability("ir"))
    }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), false)
    ...
    fun emulateUid(uid: String? = null) {
        ...
        viewModelScope.launch {
            if (!connectionManager.bruceLinkReady.value) {
                _uiState.update { it.copy(estadoOperacion = "Espera CLI OK antes de emular UID.") }
                return@launch
            }
    ...
    fun sendIr(cmd: String) {
        viewModelScope.launch {
            ...
            if (!connectionManager.bruceLinkReady.value) {
                _uiState.update { it.copy(estadoOperacion = "Espera CLI OK antes de enviar IR.") }
                return@launch
            }
            val match = Regex("""(?i)^ir\s+tx\s+(\w+)\s+([0-9a-f]+)\s+([0-9a-f]+)$""").find(normalized)
            if (match == null) {
                connectionManager.sendBruceCliLine(normalized).fold(
                    onSuccess = { _uiState.update { it.copy(estadoOperacion = "TX OK: $normalized") } },
                    onFailure = { err ->
                        _uiState.update { it.copy(estadoOperacion = "ERROR IR: ${err.message}") }
                    }
                )
                return@launch
            }
    ...
    fun captureIr() {
        viewModelScope.launch {
            if (!connectionManager.bruceLinkReady.value) {
                _uiState.update { it.copy(estadoOperacion = "Espera CLI OK antes de capturar IR.") }
                return@launch
            }
            _uiState.update { it.copy(estadoOperacion = "Capturando IR (10s) en el T-Embed…") }
            connectionManager.captureIrCompanion(10).fold(
                onSuccess = { msg ->
                    _uiState.update { it.copy(estadoOperacion = msg) }
                },
                onFailure = { err ->
