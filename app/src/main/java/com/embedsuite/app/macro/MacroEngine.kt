...
class MacroEngine(
    ...
    private suspend fun executeCommands(macro: MacroEntity): Result<Int> {
        ...
        for (cmd in commands) {
            ...
            if (cmd.startsWith("{")) {
                dispatchLegacyJsonLine(cmd).fold(
                    onSuccess = { executed++ },
                    onFailure = { return Result.failure(Exception("Falló en '$cmd': ${it.message}")) }
                )
            } else {
                connectionManager.executeBruceCli(cmd).fold(
                    onSuccess = { executed++ },
                    onFailure = { return Result.failure(Exception("Falló en '$cmd': ${it.message}")) }
                )
            }
    ...
    /** Adaptador legacy: traduce run_action JSON al adapter Bruce CLI. Preferir líneas CLI directas. */
    private suspend fun dispatchLegacyJsonLine(cmd: String): Result<String> {
        ...
        return connectionManager.sendBruceCliLine(cmd)
    }
}
