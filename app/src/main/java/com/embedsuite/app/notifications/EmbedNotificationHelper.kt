...
object EmbedNotificationHelper {
    ...
    /** Notificación persistente con acciones rápidas mientras el T-Embed está conectado. */
    fun notifyDeviceConnected(context: Context, transportLabel: String, detail: String) {
        ensureChannel(context)
        ...
        fun actionIntent(action: String, requestCode: Int): PendingIntent =
            PendingIntent.getBroadcast(
                context,
                requestCode,
                Intent(context, EmbedQuickActionReceiver::class.java).setAction(action),
                PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
            )
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            .setSmallIcon(R.mipmap.ic_launcher)
            .setContentTitle("T-Embed · $transportLabel")
            .setContentText(detail.ifBlank { "Conectado" })
            .setPriority(NotificationCompat.PRIORITY_LOW)
            .setOngoing(true)
            .setContentIntent(openApp)
            .addAction(0, "↻ Info", actionIntent(EmbedQuickActionReceiver.ACTION_REFRESH, 1))
            .addAction(0, "Menú", actionIntent(EmbedQuickActionReceiver.ACTION_NAV_MENU, 2))
            .addAction(0, "Desconectar", actionIntent(EmbedQuickActionReceiver.ACTION_DISCONNECT, 3))
            .build()
