...
import com.embedsuite.app.engine.autopilot.NoOpAutopilotEngine
...
class AppContainer(context: Context) {
    ...
    val bruceCustomCommandRepository = BruceCustomCommandRepository(database.bruceCustomCommandDao())
    ...
    val bleScanner = com.embedsuite.app.core.ble.BleScanner(appContext)
    val wifiApManager = com.embedsuite.app.core.wifi.WifiApManager(appContext)
    val transportOrchestrator = com.embedsuite.app.core.connection.TransportOrchestrator(
        connectionManager = connectionManager,
        wifiApManager = wifiApManager
    )
    val autoDiscoveryManager = com.embedsuite.app.core.orchestrator.AutoDiscoveryManager(
        context = appContext,
        connectionManager = connectionManager
    )
    val intentOrchestrator = com.embedsuite.app.core.orchestrator.IntentOrchestrator(
        context = appContext,
        transportOrchestrator = transportOrchestrator,
        connectionManager = connectionManager
    )
    val irdbSync = com.embedsuite.app.engine.sync.IrdbSync()
    val irdbRepository = com.embedsuite.app.data.IrdbRepository(
        dao = database.irdbDao(),
        irdbSync = irdbSync
    )
    ...
    val bleGattManager = com.embedsuite.app.core.ble.BleGattManager(connectionManager.deviceBleTransport)
    ...
    val autopilotEngine: AutopilotEngine = NoOpAutopilotEngine()
    ...
    val bleTelemetryPoller = com.embedsuite.app.core.ble.BleTelemetryPoller(connectionManager, appScope)

    init {
        instance = this
        connectionManager.wifiApManager = wifiApManager
        bleTelemetryPoller.start()
        val hadMock = appPreferences.ensureRealHardwareMode()
        if (hadMock) {
            Log.i(TAG, "Mock transport desactivado — modo hardware real.")
        }
        SoundFeedback.init()
        ...
        scope.launch {
            runCatching { profileRepository.migrateLegacyProfilesIfNeeded() }
                .onFailure { e ->
