...
object TehLinkResponseParser {
    ...
    fun parseDeviceInfo(data: JSONObject): TehLinkDeviceInfo {
        ...
        data.optJSONArray("capabilities")?.let { arr ->
            for (i in 0 until arr.length()) {
                val item = arr.opt(i)
                when (item) {
                    is String -> if (item.isNotBlank()) capabilityList += item
                    else -> item?.toString()?.takeIf { it.isNotBlank() }?.let { capabilityList += it }
                }
            }
        }
        ...
        return TehLinkDeviceInfo(
            ...
            protoVer = data.optString("proto_ver").ifBlank {
                data.optInt("proto_ver", 1).toString()
            }.toIntOrNull() ?: 1,
            plugins = plugins,
            hardening = hardening,
    ...
    fun parseSoakResult(data: JSONObject): TehLinkSoakResult {
        val completedRaw = data.opt("completed")
        val completed = when (completedRaw) {
            is Boolean -> completedRaw
            is Number -> completedRaw.toInt() >= data.optInt("iterations", 0) && data.optInt("iterations", 0) > 0
            else -> data.optBoolean("completed")
        }
    ...
    fun parseActionState(data: JSONObject): TehLinkActionState {
        ...
        val otaJson = data.optJSONObject("ota_status")
        val ota = when {
            otaJson != null -> parseOtaStatus(otaJson)
            data.has("sha256_verified") || (data.has("state") && data.optString("plugin_id", "") == "ota") ->
                parseOtaStatus(data)
            else -> null
        }
        val soakJson = data.optJSONObject("soak")
        val soak = when {
            soakJson != null -> parseSoakResult(soakJson)
            data.has("iterations") && data.has("leak_bytes") -> parseSoakResult(data)
            else -> null
        }
    ...
    fun parseDeviceStatus(data: JSONObject): TehLinkDeviceStatus {
        ...
        return TehLinkDeviceStatus(
            ...
            temperatureC = when {
                data.has("temperature_c") -> data.optDouble("temperature_c").toFloat()
                data.has("temp_c") -> data.optDouble("temp_c").toFloat()
                data.has("temperature") -> data.optDouble("temperature").toFloat()
                else -> null
            }
        )
    }
    ...
    fun isTehLinkLine(line: String): Boolean {
        ...
        if (trimmed.startsWith("TEH4:")) return true
