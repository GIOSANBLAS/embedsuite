...
object Esp32UsbIds {
    ...
    /** PID custom Xibalba / LilyGO (CDC TEH-Link). */
    const val PID_XIBALBA_CDC = 0x303A

    val KNOWN_PIDS = setOf(PID_USB_JTAG, PID_USB_OTG, PID_BOOTLOADER, PID_XIBALBA_CDC)
    ...
    fun deviceScore(device: UsbDevice): Int = when {
        device.vendorId == VENDOR_ESPRESSIF && device.productId == PID_XIBALBA_CDC -> 100
    ...
    fun flashDeviceScore(device: UsbDevice): Int = when {
        ...
        device.vendorId == VENDOR_ESPRESSIF && device.productId == PID_XIBALBA_CDC -> 40
