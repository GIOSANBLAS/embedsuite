...
private fun mainTabs() = listOf(
    ...
    NavTab("tools", R.string.nav_tools, Icons.Default.Hub),
    NavTab("library", R.string.nav_library, Icons.Default.Storage),
    NavTab("forge", R.string.nav_forge, Icons.Default.Build)
)
...
fun MainScreen(
    ...
    LaunchedEffect(Unit) {
        container.autoDiscoveryManager.discover()
    ...
    val (statusText, statusColor) = when (connectionState) {
        ...
        is ConnectionState.Connected -> {
            if (bruceLinkReady) stringResource(R.string.status_cli_ok) to MatrixGreen
            else stringResource(R.string.status_usb_serial_ok) to NeonOrange
        }
            ...
            NavHost(navController, startDestination = "dashboard", Modifier.padding(innerPadding).fillMaxSize()) {
                composable("dashboard") {
                    DashboardScreen(
                        ...
                        onNavigateBleWizard = { navController.navigate("ble_wizard") { launchSingleTop = true } },
                        onNavigateWifiWizard = { navController.navigate("wifi_wizard") { launchSingleTop = true } }
                    )
                }
                composable("tools") {
                    ToolsHubScreen(
                ...
                composable("library") {
                    LibraryScreen(
                        container = container,
                        onOpenRf = { navController.navigate("rf") { launchSingleTop = true } },
                        onOpenNfc = { navController.navigate("nfc_ir") { launchSingleTop = true } },
                        onOpenIr = { navController.navigate("nfc_ir") { launchSingleTop = true } }
                    )
                }
                composable("forge") {
                    PayloadForgeScreen(
                        container = container,
                        developerMode = developerMode,
                        onOpenScripts = { navController.navigate("scripts") { launchSingleTop = true } },
                        onOpenOps = { navController.navigate("ops") { launchSingleTop = true } },
                        onOpenTerminal = { navController.navigate("terminal") { launchSingleTop = true } },
                        onOpenBruceFiles = {
                            if (developerMode) navController.navigate("bruce_files") { launchSingleTop = true }
                        },
                        onOpenTools = { navController.navigate("tools") { launchSingleTop = true } }
                    )
                }
                composable("bruce_files") {
                    BruceFilesScreen(
                        container = container,
                        onBack = { navController.popBackStack() }
                    )
                }
                composable("ble_wizard") {
                    BlePairingWizardScreen(
                        container = container,
                        onBack = { navController.popBackStack() },
                        onConnected = { navController.popBackStack() }
                    )
                }
                composable("wifi_wizard") {
                    WifiPairingWizardScreen(
                        container = container,
                        onBack = { navController.popBackStack() },
                        onConnected = { navController.popBackStack() }
                    )
                ...
                composable("ir_search") {
                    IrSearchScreen(
                        viewModel = viewModel(factory = viewModelFactory),
                        onBack = { navController.popBackStack() }
                    )
                }
                composable("subghz_analyzer") {
                    SubGhzAnalyzerScreen(
                        viewModel = viewModel(factory = viewModelFactory),
                        onBack = { navController.popBackStack() }
                    )
                }
                composable("badusb_forge") {
                    BadUsbForgeScreen(
                        viewModel = viewModel(factory = viewModelFactory),
                        onBack = { navController.popBackStack() }
                    )
                }
                composable("ir_finder") {
                    IrFinderScreen(
                        viewModel = viewModel(factory = viewModelFactory),
                        onBack = { navController.popBackStack() }
                    )
                }
                composable("spam_generator") {
                    SpamGeneratorScreen(
                        viewModel = viewModel(factory = viewModelFactory),
                        onBack = { navController.popBackStack() }
                    )
