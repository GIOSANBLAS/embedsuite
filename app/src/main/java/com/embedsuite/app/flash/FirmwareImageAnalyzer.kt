...
/**
 * Detecta si un .bin es imagen merged (bootloader+partitions+app @ 0x0, estilo Bruce/Bruce)
...
object FirmwareImageAnalyzer {
    ...
    /** Offsets estándar ESP32-S3 / LilyGO T-Embed 16 MB (compatible Bruce + Bruce). */
    ...
    fun analyze(file: File): Analysis {
        ...
        val warning = when {
            merged -> null
            meta?.version?.contains("0.20") == true || meta?.version?.contains("0.19") == true || meta?.version?.contains("Bruce", ignoreCase = true) == true ->
                "Binario APP (${meta.version}) — se flashea en 0x10000. " +
                    "El release oficial Bruce-0.20.1 es merged @ 0x0 (estilo Bruce)."
            meta != null ->
                "Binario APP (${meta.version}) — se flashea en 0x10000. " +
                    "Para instalación limpia usa el merged .bin @ 0x0 de pr3y/Bruce."
            !merged ->
