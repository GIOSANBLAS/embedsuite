...
object SmartOtaGuard {
    ...
    suspend fun postFlashHealthCheck(cm: DeviceConnectionManager): HealthResult {
        if (cm.connectionState.value !is ConnectionState.Connected) {
            cm.connect(TransportType.USB)
            delay(2_000)
        }

        val pingRaw = cm.sendTehLinkRaw("""{"cmd":"ping","id":1}""")
        val pingOk = pingRaw.isSuccess

        val infoRaw = cm.sendTehLinkRaw("""{"cmd":"get_info","id":2}""")
        val xibalbaDetected = cm.detectedProfile.value == FirmwareProfile.XIBALBA ||
            infoRaw.getOrNull()?.contains("xibalba", ignoreCase = true) == true ||
            infoRaw.getOrNull()?.contains("teh-link", ignoreCase = true) == true

        cm.refreshSystemInfo()

        val healthy = pingOk && xibalbaDetected
        val message = when {
            healthy -> "Dispositivo responde y perfil Xibalba confirmado."
            !pingOk -> "Ping TEH-Link falló tras el flasheo."
            !xibalbaDetected -> "get_info no confirma perfil Xibalba."
        ...
        return HealthResult(
            healthy = healthy,
            pingOk = pingOk,
            xibalbaDetected = xibalbaDetected,
            message = message
        )
    }
