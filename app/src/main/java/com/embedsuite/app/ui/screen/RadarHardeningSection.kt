...
fun RadarHardeningSection(
    ...
    if (systemInfo.profile != FirmwareProfile.BRUCE) return

    val blips = remember(systemInfo.bruceCapabilities, systemInfo.hardening) {
...
private fun buildRadarBlips(systemInfo: SystemInfo): List<RadarBlip> {
    val caps = systemInfo.bruceCapabilities
